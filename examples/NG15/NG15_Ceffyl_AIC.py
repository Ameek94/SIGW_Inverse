...
def psd(f,Tspan,get_gwb_func,free_nodes,left_node,right_node,**kwargs):
    ...
    # use kwargs to build nodes and values, xs and ys are passed as arrays so need to take care
    # if free_nodes>1:
...
def log_likelihood(params,pta=None,prior_transform=None):
    # print(params)
    # print(params.shape)
...
# Global variable to track the best fit
best_f = np.inf
best_x = None

def print_fun(x, f, accepted):
    global best_f, best_x

    # Update the best fit found so far
    if f < best_f:
        best_f = f
        best_x = np.copy(x)

    print(f"At minimum: {f:.4f}, Accepted: {int(accepted)}")
    print(f"Best fit so far: {best_f:.4f}, Best parameters: {best_x}")
...
def main():
    ...
    prior = partial(prior1D,
                    ...
                    y_min=y_min,
                    y_max=y_max)
    loglike = partial(log_likelihood,pta=pta,prior_transform=prior)    
    # Optimize instead of sample
    ...
    for i in range(niter):
        x0 = np.random.uniform(0,1,ndim) # tweak for batched acquisition
        soln = solve(loglike,x0,bounds=(lower,upper)
                          ,seek_global_minimum=True,print_progress=True,do_logging=False)
        if soln.f < best_f:
            best_f = soln.f
            best_x = soln.x
        print(f"At minimum: {soln.f:.4f}, parameter: {soln.x}")
        print(f"Best fit so far: {best_f:.4f}, Best parameters: {best_x}")
    # results = basinhopping(loglike,x0=x0,stepsize=0.07,niter=2000,minimizer_kwargs=minimizer_kwargs
    #                        , callback=None)
    x = prior(best_x)
    y = best_f
    print("Optimization results")
    print(f"Best fit: {y}")
    print(f"Best parameters: {x}")

    # print("Optimization results")
    # print(f"Best fit: {results.fun}")
    # y = results.fun
    # x = prior(results.x)
    # print(f"Best parameters: {x}")
    # # print(f"Best parameters: {results.x}")

    np.savez(f'best_fit_{num_nodes}.npz',best_fit=y,best_params=x)
    ...
    # start = time.time()
    # sampler.run(verbose=True,f_live=0.005)
    # end = time.time()
