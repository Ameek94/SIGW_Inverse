...
def I_sq_RD(t, s, k):
    ...
    """
    prefactor = 288 * (-5 + s**2 + t * (2 + t))**2/ \
        ((1 - s + t)**6 * (1 + s + t)**6)
    log_term = ((-1 + s - t) * (1 + s + t) 
                + ((-5 + s**2 + t * (2 + t)) 
                * jnp.log(jnp.abs((-2 + t * (2 + t)) / (3 - s**2)))) / 2.)**2
    heaviside_term = (jnp.pi**2 * (-5 + s**2 + t * (2 + t))**2 
                      * jnp.heaviside(1 - jnp.sqrt(3) + t, 1)) / 4.
...
# We precompute the part of the Large V contribution to the early matter domination
# kernel that contains the Si and Ci trigonometric integrals as there is no jit-able 
# implementation of these functions in jax.
...
cos = jnp.cos(xvals / 2.)
sin = jnp.sin(xvals/2.)
LV = jnp.log(4. * ci**2. + (jnp.pi - 2. * si)**2)
@jit
def _sici_precomp(x):
    """
...
# below are the two main contributions to the transitioning kernel, based on sudden-reheating scenarios.
# As they are evaluated at different t's we need to evaluate them separately and sum them up.
...
def I_sq_IRD_LV(t, s, k, kmax, etaR):
    ...
    to the transitioning kernel from an early matter dominated era to radiation domination.
    ...
    s_max = jnp.where(xR <= xmaxR, 1., 2. * xmaxR_ratio - 1.)
    t_max = -s + 2. * xmaxR_ratio - 1.
    ...
    result = jnp.where(valid_t & valid_s, 
                       (9. * t**4. * xR**8. * _sici_precomp(xR)) / 81920000., 
                       0.0)
    result = (9. * t**4. * xR**8. * _sici_precomp(xR)) / 81920000.
...
def d_I_sq_IRD_LV(index, t, s, k, kmax, etaR):
    ...
    with respect to `kmax` or `etaR` based on `idx`. 

    .. note::
    The gradient w.r.t `kmax` is zero even though the kernel depends on it through the integration limits.
    This is handled in the integration function.

    Parameters:
    - index: int
        Index of the parameter to differentiate with respect to (0 for kmax, 1 for etaR).
    - t: jax.numpy.ndarray
        Array of t values.
    - s: jax.numpy.ndarray
        Array of s values.
...
def I_sq_IRD_res(t, s, k, kmax, etaR):
    ...
    to the transitioning kernel from an early matter dominated era to
...
def d_I_sq_IRD_res(index, t, s, k, kmax, etaR):
    ...
    grad_etaR = 7/etaR * result # Based on the simplified result of the derivative
...
def simpson_uniform(f, x):
    ...
    If f is a multi-dimensional array, the integration is performed over the first axis.
    ...
    result = f[0] + f[-1] + 2 * jnp.sum(f[2:N:2], axis=0) + 4 * jnp.sum(f[1:N:2], axis=0)
    ...
    if N % 2 == 0:
        result -= (f[-2] + f[-1]) * h / 3  # Subtract last interval computed in the main sum
        result += (h / 2) * ((2 * f[-1] + f[-2]) + (f[-1] + f[-2]))  # Trapezoidal rule for the last interval
...
def simpson_nonuniform(f, x):
    ...
    This fully vectorized implementation is suitable for multi-dimensional arrays,
    integrating over the first axis. Assumes non-uniformly spaced grid points.
    ...
    hdh = h1 / h0
    hmh = h1 * h0
    result = jnp.sum((hph / 6) * ((2 - hdh) * f[:-2:2] +
                                  (hph ** 2 / hmh) * f[1:-1:2] +
                                  (2 - 1 / hdh) * f[2::2]), axis=0)

    # Additional computation for even N (last segment)
    if N % 2 == 1:
        h0 = h[-2]
        h1 = h[-1]
        result += (f[-1] * (2 * h1 ** 2 + 3 * h0 * h1) /
                   (6 * (h0 + h1)))
        result += (f[-2] * (h1 ** 2 + 3 * h1 * h0) /
                   (6 * h0))
        result -= (f[-3] * h1 ** 3 /
                   (6 * h0 * (h0 + h1)))
...
def integrand(P_zeta, kernel, s, t, k, *params):
    ...
    """
    return kernel(t, s, k) * polynomial(t, s) * \
...
def integrand_transitioning_kernel(P_zeta, kernel, s, t, k, *params):
    ...
    kmax = params[-2]
    etaR = params[-1]
    return kernel(t, s, k, kmax, etaR) * polynomial(t, s) * \
        P_zeta(k*u(t, s), *params) * \
        P_zeta(k*v(t, s), *params)
jitted_integrand_transitioning_kernel = jit(integrand_transitioning_kernel, static_argnums=[0,1])
...
def d_integrand(index, P_zeta, dP_zeta, kernel, s, t, k, *params):
    ...
    """
    return kernel(t, s, k) * polynomial(t, s) * \
        (P_zeta(k* u(t, s), *params) * dP_zeta(P_zeta, index, k* v(t, s), *params) + \
        dP_zeta(P_zeta, index, k* u(t, s), *params) * P_zeta(k* v(t, s), *params))

def d_integrand_transitioning_kernel(index, P_zeta, dP_zeta, kernel, d_kernel, s, t, k, *params):
    """
    Derivative of the integrand with respect to a parameter for the
    ...
    term1 = kernel(t, s, k, kmax, etaR) * polynomial(t, s) * \
        (P_zeta(k* u(t, s), *params) * dP_zeta(P_zeta, index, k* v(t, s), *params) + \
        dP_zeta(P_zeta, index, k* u(t, s), *params) * P_zeta(k* v(t, s), *params))
    if index != len(params)-1:
        ...
        term2 = d_kernel(1, t, s, k, kmax, etaR) * polynomial(t, s) * \
            P_zeta(k* u(t, s), *params) * P_zeta(k* v(t, s), *params)
        return term1 + term2
    
def d_integrand_transitioning_kernel_delta(index, P_zeta, dP_zeta, kernel, d_kernel, s, t, k, *params):
    """
    Derivative of the integrand with respect to a parameter for the
    transitioning kernels.
    ...
    if index == len(params)-2:
        term1 = kernel(t, s, k, kmax, etaR) * polynomial(t, s) * \
            dP_zeta(P_zeta, index, k* v(t, s), *params)**2.
    else:
        term1 = kernel(t, s, k, kmax, etaR) * polynomial(t, s) * \
            (P_zeta(k* u(t, s), *params) * dP_zeta(P_zeta, index, k* v(t, s), *params) + \
            dP_zeta(P_zeta, index, k* u(t, s), *params) * P_zeta(k* v(t, s), *params))
...
def dP_zeta_auto(P_zeta, index, k, *params):
    """
    Automatic gradient of the primordial power spectrum function. This only works for
    jax functions.
    """
    args = (k, *params)
    tangent_vector = tuple(jnp.zeros_like(arg) if i != index+1 
                           else jnp.ones_like(arg) for i, arg in enumerate(args))
...
class OmegaGWjax():
    ...
    def __init__(self, s, t, f=None, norm="RD", kernel="RD", upsample=False, dP_zeta="auto", to_numpy=False, jit=True,
        ...
        if norm=="RD":
            self.norm = lambda k: CG/24. * OMEGA_R
        elif norm=="CT":
            self.norm = lambda k: 1./24.
        elif callable(norm):
            self.norm = norm
        else:
            raise ValueError("Norm should be a callable, 'RD' or 'CT'. Other "
                             "norms are not supported right now.")
        
        if jit:
            integrands = [jitted_integrand, jitted_integrand_transitioning_kernel]
        else:
            integrands = [integrand, integrand_transitioning_kernel]

        d_integrands = [d_integrand, d_integrand_transitioning_kernel, d_integrand_transitioning_kernel_delta]
        ...
        if kernel == "RD":
            ...
            if len(kernel) == 2:
                ...
                raise ValueError("If kernel is an iterable, it should have "
                                 "length 2. Got length {}".format(len(kernel)
                                    ))
        else:
            raise ValueError("Supported kernels are 'RD', 'I_MD_to_RD', a callable or "
                             "an iterable containing exactly 2 kernel functions. Got {}".format(kernel))
    ...
    def integrate_constant_kernel(self, P_zeta, s, t, kvec, *params):
        ...
        """
        integrand_values = self.integrand(P_zeta, self.kernel, s[:, None, None], t[None, :, :], 
                                            kvec[None, None, :], *params)
    ...
    def integrate_transitioning_kernel(self, P_zeta, s, t, kvec, *params):
        ...
        - jax.numpy.ndarray
            Array of :math:`\overline{\mathcal{P}_h}` values.
        """
        lV_values = self.integrand(P_zeta, self.kernel[0], 
                                   s[:, None, None], t[None, :, :], 
                                   kvec[None, None, :], *params)
        lV_integrated = simpson_uniform(lV_values, s)
        lV_integrated = simpson_nonuniform(lV_integrated, t)
        t_res = jnp.sqrt(3)-1.
        res_values = self.integrand(P_zeta, self.kernel[1], 
                                    s[:, None], t_res, 
                                    kvec[None, :], *params)
    ...
    def d_integrate(self, index, fvec, *params):
        ...
        res = self.d_integration_routine(index, self.P_zeta, self.dP_zeta, s, t, kvec, *params)
        out = 2 * self.norm(kvec_full_resolution) * self.upsample_k(kvec_full_resolution, kvec, res)

        return out
    
    def d_integrate_constant_kernel(self, index, P_zeta, dP_zeta, s, t, kvec, *params):
        """
        ...
        integrand_values = self.d_integrand(index, P_zeta, dP_zeta, self.kernel, s[:, None, None], t[None, :, :], 
                                            kvec[None, None, :], *params)

        s_integrated = simpson_uniform(integrand_values, s)
        return simpson_nonuniform(s_integrated, t)
    
    def d_integrate_transitioning_kernel(self, index, P_zeta, dP_zeta, s, t, kvec, *params):
        """
        Compute the derivative of :math:`Omega_{GW}` values with respect
        to params[index].
        ...
        # For kmax we need to fix t because of the theta functions in the kernel and the derivative of
        # the resonant contribution is zero
        if index == len(params)-2 and self.dP_zeta_has_delta:
            t = (-kvec[None, :] + 2*kmax - kvec[None, :]*s[:, None])/kvec[None, :]
            lV_values = self.d_integrand(index, P_zeta, dP_zeta, self.kernel[0], self.d_kernel[0],
                                        s[:, None], t, kvec[None, :], *params) 
            lV_values = lV_values * jnp.where(t >= 0, 1, 0)
            lV_integrated = simpson_uniform(lV_values, s)
            t = jnp.sqrt(3)-1.
            s = (-kvec + 2*kmax - kvec*t)/kvec
            res_values = self.d_integrand(index, P_zeta, dP_zeta, self.kernel[1], self.d_kernel[1],
                                        s, t, kvec, *params)
            res_values = res_values *jnp.where(2 * kmax - kvec * (1 + t) >= 0, 1, 0) * jnp.where((1 - s >= 0) & (s + 1 >= 0), 1, 0)
        ...
        # For all other derivatives we just proceed as usual
            ...
            lV_values = self.d_integrand(index, P_zeta, dP_zeta, self.kernel[0], self.d_kernel[0], 
                                         s[:, None, None], t[None, :, :], kvec[None, None, :], *params)
            lV_integrated = simpson_uniform(lV_values, s)
            lV_integrated = simpson_nonuniform(lV_integrated, t)
            t_res = jnp.sqrt(3)-1.
            res_values = self.d_integrand(index, P_zeta, dP_zeta, self.kernel[1], self.d_kernel[1],
                                          s[:, None], t_res, kvec[None, :], *params)
            res_integrated = simpson_uniform(res_values, s)
            return lV_integrated + res_integrated
        
    def d_integrate_transitioning_kernel_analytical(self, index, P_zeta, s, t, fvec, *params):
        ...
        if index == len(params)-2: # kmax
            t1 = (-kvec[None, :] + 2*kmax - kvec[None, :]*s[:, None])/kvec[None, :]
            lV_values = I_sq_IRD_LV(t1, s[:, None], kvec[None, :], kmax, etaR) * polynomial(t1, s[:, None])
            lV_values = lV_values * jnp.where(t1 >= 0, 1, 0) # * jnp.where( kvec[None, :]*s[:, None] >= 0, 1, 0)
            lV_integrated = simpson_uniform(lV_values, s)
            lV_integrated = lV_integrated * As**2  * 2  * 2/kvec

            t2 = jnp.sqrt(3)-1.
            s2 = (-kvec + 2*kmax - kvec*t2)/kvec
            res_values = I_sq_IRD_res(t2, s2, kvec, kmax, etaR) * polynomial(t2, s2) 
            res_values = res_values * jnp.where(2 * kmax - kvec * (1 + t2) >= 0, 1, 0) * jnp.where((1 - s2 >= 0) & (s2 + 1 >= 0), 1, 0)
    ...
    def integrate_single_k(self, f, *params):
        ...
        integrand_values = integrand(self.P_zeta, self.kernel, s[:, None, None], t[None, :, None], 
                                     k, *params)
