...
def plot_functional_posterior(vals=[], k_arr=[], intervals=[99.7, 95., 68.],
                              weights = None,
                              ylabels=[r'$P_{\zeta}$', r'$\log_{10} \Omega_{\rm GW}$'],
                              aspect_ratio=(6, 5),
...
print(f"frequencies from violin_data: {freqs}")
print(f"frequencies from data_dir: {frequencies}")
...
pattern = re.compile(rf'samples_(\d+)_linear\.npz')
...
if matching_files:
    ...
    for file in matching_files:
        ...
        ys = jnp.array(ys)
        if is_2d:
            l = jnp.ones((len(ys),1)) * left_node
            r = jnp.ones((len(ys),1)) * right_node
        ...
        pz, gwb = split_vmap(get_pz_omega, (xs, ys), batch_size=200)
...
log_gwb_samples = np.log10(gwb_samples)

fig, (ax1,ax2) = plot_functional_posterior([pz_samples,log_gwb_samples],
                                    k_arr=[p_arr_local, np.log10(frequencies)],
                                    weights = weights,
                                    aspect_ratio=(6,4.5))
...
#     secax = x.secondary_xaxis('top', functions=(lambda x: x * k_mpc_f_hz, lambda x: x / k_mpc_f_hz))
#     secax.set_xlabel(r"$k\,{\rm [Mpc^{-1}]}$",labelpad=10) 
plt.savefig(f'./NG15_linear_posterior.pdf',bbox_inches='tight')