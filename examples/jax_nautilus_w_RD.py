...
def main():
    ...
    sampler = Sampler(prior, likelihood, ndim, pass_dict=False, vectorized=True
                                            ,pool=(None,4),filepath=f'./{gwb_model}_wfld_free_{num_nodes}_wRD.h5') 

    start = time.time()
    sampler.run(verbose=True, f_live=0.001, n_like_max=1e6)#, n_eff=2000*ndim)
    end = time.time()
    ...
    np.savez(f'{gwb_model}_wfld_free_{num_nodes}_wRD.npz', samples=samples, logl=logl, logwt=logwt, logz=sampler.log_z)
    print(samples.shape)
    print(logl.shape)
    print(logwt.shape)
