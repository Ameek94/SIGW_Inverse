...
data = data = np.load(f'../bpl_{model}_data.npz')
frequencies = data['k']
Omegas = data['gw']
cov = data['cov']
p_arr = data['p_arr']
...
pattern = re.compile(rf'nautilus_bpl_{model}_(\d+)_linear_nodes\.npz')
...
plt.savefig(f'./nautilus_bpl_{model}_linear_posterior.pdf',bbox_inches='tight')