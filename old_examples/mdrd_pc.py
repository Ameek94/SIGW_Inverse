...
kstar = 1e-3
gwb_sigma = gwb_amp*( 0.1*(np.log(k_arr/kstar))**2 + 0.05) # 2% error at kstar + more towards edges
gwb_cov = jnp.diag(gwb_sigma**2)
...
def prior(x):
    ...
    # return x
    return UniformPrior(-5,5)(x)

# maybe jax does not work with pc?
def loglikelihood(params):
    ...
    r2 = 0.
    return -0.5*sum( (gwb-gwb_amp)**2 / gwb_sigma**2) , [r2]
    # return -sum(params)**2, [r2]
