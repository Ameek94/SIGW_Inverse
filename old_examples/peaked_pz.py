...
t = jnp.logspace(-4,4, 150)  # Second rescaled internal momentum
...
@jit
def pz(p,pstar=5e-4,sigma=0.1,amp=1e-1,floor=1e-3):
...
mc_sample_kwargs = { 'num_warmup': 512,'num_samples': 1024,'verbose': True }
osc = sampler(pz_kmin=kmin,pz_kmax=kmax,gwb_karr=k_arr,
                ...
                y_low=-8,y_high=1,interpolation_method='CubicSpline',min_nodes=3,max_nodes=10
...
peaked = sampler(pz_kmin=kmin,pz_kmax=kmax,gwb_karr=k_arr,
                 gwb_means=gwb_amp,gwb_cov=gwb_cov,
                 gwb_method='jax',gwb_method_kwargs=gwb_method_kwargs,
                y_low=-8.,y_high=1,interpolation_method='CubicSpline',min_nodes=3,max_nodes=10,aic=False,
