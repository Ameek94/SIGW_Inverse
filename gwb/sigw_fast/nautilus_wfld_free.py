...
def main():
    ...
    sampler = Sampler(prior_transform, loglikelihood, ndim, pass_dict=False,filepath=f'{gwb_model}_wfld_free_{num_nodes}.h5',pool=(None,4))

    sampler.run(verbose=True, f_live=0.01,n_like_max=int(5e6))
    ...
    np.savez(f'{gwb_model}_wfld_free_{num_nodes}.npz', samples=samples, logl=logl, logwt=logwt,logz=sampler.log_z)
