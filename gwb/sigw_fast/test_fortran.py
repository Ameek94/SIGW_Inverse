...
def main():
    ...
    for i in range(niter):
        omega_gw1 = compute(test_pz, frequencies, Use_Cpp=False)
    end = time.time()
    ...
    start = time.time()
    for i in range(niter):
        omega_gw2 = compute(test_pz, frequencies, Use_Cpp=True)
    end = time.time()
    ...
    start = time.time()
    for i in range(niter):
        omega_gw3 = compute_rd(frequencies,use_mp=False)
    end = time.time()
    ...
    plt.loglog(frequencies, omega_gw1, label="Full python version", lw=3,alpha=0.5)
