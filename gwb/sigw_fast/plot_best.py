...
best_sample = np.atleast_2d(samples[best_idx])

print(f'Best sample: {best_sample}')

best_w = best_sample[0,0]
best_nodes = best_sample[0,1:num_nodes-1]
best_nodes = np.pad(best_nodes, (1,1), 'constant', constant_values=(left_node, right_node))
best_vals = best_sample[0,num_nodes-1:]
print(f'Best w: {best_w},  Best nodes: {best_nodes}, Best vals: {best_vals}')

# plot best sample

fig, ax = plt.subplots(1,2,figsize=(12,6),constrained_layout=True)
pz_best = compute_pz(p_arr, best_sample)
ax[0].loglog(p_arr_data, pz_amps, color='k', lw=1.5)
ax[0].loglog(p_arr, pz_best[0], color=blue, lw=2)
ax[0].set_ylabel(r'$P_{\zeta}$')
ax[0].scatter(10**best_nodes, 10**best_vals, color=blue, s=20, label='Best sample')

omgw_best = compute_w(frequencies, best_sample)
ax[1].loglog(frequencies, Omegas, color='k', lw=1.5, label='Truth')
ax[1].loglog(frequencies, omgw_best[0], color=blue, lw=2)
