...
def main():
    ...
    sampler = Sampler(prior_transform, loglikelihood, ndim, pass_dict=False,
                      filepath=f'{gwb_model}_w0p5_free_{num_nodes}.h5',pool=(None,8))
    ...
    np.savez(f'{gwb_model}_w0p5_free_{num_nodes}.npz', samples=samples, logl=logl, logwt=logwt,logz=sampler.log_z,omegagw=blobs)
