...
class custom_likelihood(external_likelihood):
    ...
    def get_initial_points(self, n_init_sobol=8, n_cobaya_init=0):
        ...
        bf_vals = np.log10(self.true_pz(p = 10**bf_nodes)) + 0.089
        bf_vals = np.clip(bf_vals,self.y_min,self.y_max)
...
def main():
    ...
    sigw_likelihood = custom_likelihood(free_nodes=free_nodes,left_node=left_node,right_node=right_node,
                ...
                loglikelihood = wrapped_likelihood
                 ...
                 ,name = f'{gwb_model}_{num_nodes}_bo',
                 minus_inf = minus_inf)

    start = time.time()
    sampler = BOBE(n_sobol_init = 256, 
        miniters=200, maxiters=1200,max_gp_size=1200,
        ...
        lengthscale_priors='DSLP', use_svm=True,minus_inf=minus_inf,svm_threshold=150,svm_gp_threshold=5000,)
