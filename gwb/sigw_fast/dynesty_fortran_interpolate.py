...
nk = len(frequencies)
def compute_rd(nodes,vals,frequencies,use_mp=False,nd=nd):
...
nlive = max(25*ndim, 300)
sampler = NestedSampler(likelihood, prior, ndim, nlive=nlive, sample='rslice')
...
# save samples and logprob
np.savez(f"{gwb_model}_dynesty_sigwfast_{num_nodes}.npz",samples=samples,lp=lp)
...
plt.savefig(f"{gwb_model}_dynesty_sigwfast_{num_nodes}.pdf")