...
def main():
    ...
    sampler = Sampler(prior_transform, loglikelihood, ndim, pass_dict=False,filepath=f'{gwb_model}_wfld_fixed_0p66_{num_nodes}.h5')
    ...
    np.savez(f'{gwb_model}_wfld_fixed_0p66_{num_nodes}.npz', samples=samples, logl=logl, logwt=logwt,logz=sampler.log_z)
    ...
    print(f"Max and min logprob: {np.max(logl)}, {np.min(logl)}")
