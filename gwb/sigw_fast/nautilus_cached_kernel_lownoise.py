...
def main():
    ...
    kstar = 1e-3
    omks_sigma_0 = Omegas * (0.05 * (np.log(frequencies / kstar))**2 + 0.1)
    omks_sigma = Omegas * (0.01 * (np.log(frequencies / kstar))**2 + 0.05)
    cov = np.diag(omks_sigma**2)

    # plt.loglog(frequencies, Omegas, color='k', label='GWB data',lw=1.5)
    # plt.errorbar(frequencies, Omegas, yerr=omks_sigma_0,fmt='none',ecolor='r',capsize=2,label='GWB data with error')
    # plt.errorbar(frequencies, Omegas, yerr=omks_sigma, fmt='none',ecolor='b',capsize=2, label='GWB data with error')
    ...
    sampler = Sampler(prior_transform, loglikelihood, ndim, pass_dict=False,resume=True,
                      filepath=f'{gwb_model}_{data_file}_free_{num_nodes}_lownoise.h5',pool=(None,4))
    ...
    np.savez(f'{gwb_model}_{data_file}_free_{num_nodes}_lownoise.npz', samples=samples, logl=logl, logwt=logwt,logz=sampler.log_z,omegagw=blobs)
    ...
    # Plot 1D posterior
    w = 2/3
    weights = np.exp(logwt - logsumexp(logwt))
    ...
    # plt.show()
    g.export(f'{gwb_model}_0p66_{num_nodes}_1D_w_lownoise.pdf')
    # ax = g.subplots[0,0]
